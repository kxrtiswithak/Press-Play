<img src="src/main/resources/static/images/press-play-favicon.png" width=150 align="right" />

# Press-Play ![sparta_badge][sparta_badge]
> Online video store

A group project for Sparta Global that lasted for one week. We were tasked with the challenge of providing physical (theoretcial) video store, Press play, an online presence so that they could adjust to current circumstances and still be able to offer their customers the same options as visiting the store.

---


[sparta_badge]: https://img.shields.io/badge/project-sparta-%23e43560?label=project&logo=data:image/png;base64,
