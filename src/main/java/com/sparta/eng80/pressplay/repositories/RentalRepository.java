...
public interface RentalRepository extends CrudRepository<RentalEntity, Integer> {
    ...
    @Query(value = "SELECT * FROM rental r JOIN inventory i ON r.inventory_id = i.inventory_id WHERE r.customer_id = ?1 AND i.is_rented = true AND r.return_date <= ?2 AND (SELECT count(*) FROM rental r2 WHERE r2.inventory_id = r.inventory_id AND r2.return_date >= r.return_date) <= 1", nativeQuery = true)
    Iterable<RentalEntity> findMostRecentRentalsForCustomerGroupedByInventoryIdWhereReturnDateIsBefore(int id, Timestamp date);
    @Query(value = "SELECT * FROM rental r JOIN inventory i ON r.inventory_id = i.inventory_id WHERE i.is_rented = true AND r.return_date <= ?1 AND (SELECT count(*) FROM rental r2 WHERE r2.inventory_id = r.inventory_id AND r2.return_date >= r.return_date) <= 1", nativeQuery = true)
